...
class BaseDownloadApiViewsTest(BaseDownloadTests, BaseAPITestCase):
    ...
    def test_get_os(self):
        ...
        self.assertIn(self.create_url("os", self.linux.pk), content[0]["resource_uri"])
    ...
    def test_post_release(self):
        release_page = PageFactory(title="python 3.3", path="/rels/3-3/", content="python 3.3. released")
    ...
    def test_filter_release(self):
        ...
        content = self.get_json(response)
        self.assertEqual(len(content), 1)
        self.assertIn(self.create_url("release", self.pre_release.pk), content[0]["resource_uri"])
        ...
        self.assertEqual(content[0]["release_notes_url"], self.pre_release.release_notes_url)
    ...
    def test_filter_release_file(self):
        ...
        response = self.client.get(self.create_url("release_file", filters={"description": "windows"}))
        self.assertEqual(response.status_code, 400)
        content = self.get_json(response)
        self.assertIn("error", content)
        self.assertIn("'exact' is not an allowed filter on the 'description' field.", content["error"])

        response = self.client.get(self.create_url("release_file", filters={"description__contains": "Windows"}))
        self.assertEqual(response.status_code, 200)
        content = self.get_json(response)
        self.assertEqual(len(content), 2)

        response = self.client.get(self.create_url("release_file", filters={"name": self.release_275_linux.name}))
        self.assertEqual(response.status_code, 200)
        content = self.get_json(response)
        self.assertEqual(len(content), 1)

        response = self.client.get(self.create_url("release_file", filters={"os": self.windows.pk}))
        self.assertEqual(response.status_code, 200)
        content = self.get_json(response)
        self.assertEqual(len(content), 2)
