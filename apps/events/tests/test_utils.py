...
from apps.events.utils import minutes_resolution, seconds_resolution, timedelta_nice_repr, timedelta_parse
...
class EventsUtilsTests(TestCase):
    ...
    def test_timedelta_nice_repr(self):
        ...
        for timedelta, arguments, expected in tests:
            with self.subTest(timedelta=timedelta, arguments=arguments):
                self.assertEqual(timedelta_nice_repr(datetime.timedelta(**timedelta), *arguments), expected)
    ...
    def test_timedelta_parse(self):
        tests = [
            ...
            ("-1 weeks, 2 days, -3 hours, 4 minutes, -5 seconds", datetime.timedelta(-5, 11045)),
