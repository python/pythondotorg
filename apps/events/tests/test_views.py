...
from apps.events.models import Calendar, Event, EventCategory, EventLocation, OccurringRule, RecurringRule
from apps.events.templatetags.events import get_events_upcoming
...
class EventsViewsTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.event_starts_at_future_year = Event.objects.create(
            title="Event Starting Following Year", creator=cls.user, calendar=cls.calendar
        )
        cls.event_ends_at_future_year = Event.objects.create(
            title="Event Ending Following Year", creator=cls.user, calendar=cls.calendar
        )
        ...
        cls.future_event = Event.objects.create(
            title="Future Event", creator=cls.user, calendar=cls.calendar, featured=True
        )
        ...
        cls.rule_single_day = OccurringRule.objects.create(
            event=cls.event_single_day, dt_start=recurring_time_dtstart, dt_end=recurring_time_dtstart
        )
    ...
    def test_event_list_category(self):
        category = EventCategory.objects.create(name="Sprints", slug="sprints", calendar=self.calendar)
    ...
    def test_event_starting_future_year_displays_relevant_year(self):
        ...
        url = reverse("events:events")
        response = self.client.get(url)
        self.assertIn(f'<span id="start-{event.id}">', response.content.decode())
    ...
    def test_event_ending_future_year_displays_relevant_year(self):
        ...
        url = reverse("events:events")
        response = self.client.get(url)
        self.assertIn(f'<span id="end-{event.id}">', response.content.decode())
    ...
    def test_events_scheduled_current_year_does_not_display_current_year(self):
        ...
        self.assertIn(  # start date
            f'<span id="start-{event.id}" class="say-no-more">', response.content.decode()
        )
