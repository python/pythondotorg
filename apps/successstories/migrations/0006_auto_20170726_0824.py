...
from apps.successstories.utils import convert_to_datetime, get_field_list
...
def migrate_old_content(apps, schema_editor):
    ...
    pages = Page.objects.using(db_alias).filter(
        path__startswith="about/success/", content_markup_type="restructuredtext"
    )
    stories = []
    for page in pages.iterator():
        field_list = dict(get_field_list(page.content.raw))
        extract_company_name = page.path.split("/")[-1]
        company_name = normalized_company_names.get(extract_company_name.lower(), extract_company_name.title())
