...
from apps.users.forms import MembershipForm, UserProfileForm
...
class UsersFormsTestCase(TestCase):
    def test_signup_form(self):
        form = SignupForm(
            {"username": "username", "email": "test@example.com", "password1": "password", "password2": "password"}
        )
    ...
    def test_password_mismatch(self):
        ...
        self.assertEqual(form.errors["password2"], ["You must type the same password each time."])

    def test_duplicate_username(self):
        User.objects.create_user("username2", "test@example.com", "testpass")

        form = SignupForm(
            {"username": "username2", "email": "test2@example.com", "password1": "password", "password2": "password"}
        )
        self.assertFalse(form.is_valid())
    ...
    def test_user_membership(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors["psf_code_of_conduct"], ["Agreeing to the code of conduct is required."])
...
class UserProfileFormTestCase(TestCase):
    def test_unique_email(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors, {"email": ["Please use a unique email address."]})
