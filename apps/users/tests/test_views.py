...
from apps.sponsors.models import RequiredTextAssetConfiguration, SponsorBenefit, Sponsorship
    ...
    def test_usernames(self):
        ...
        for i, username in enumerate(usernames):
            with self.subTest(i=i, username=username):
                post_data.update({"username": username, "email": f"foo{i}@example.com"})
                response = self.client.post(url, post_data, follow=True)
                self.assertEqual(response.status_code, 200)
...
class SponsorshipDetailViewTests(TestCase):
    def setUp(self):
        ...
        self.url = reverse("users:sponsorship_application_detail", args=[self.sponsorship.pk])
