    ...
    def handle(self, *args, **options):
        ...
        location_sf, _ = EventLocation.objects.get_or_create(
            ...
            defaults={
                ...
                'url': 'https://example.com/sf-venue'
            }
        )
        ...
        location_ny, _ = EventLocation.objects.get_or_create(
            ...
            defaults={
                ...
                'url': 'https://example.com/ny-venue'
            }
        )
        ...
        location_london, _ = EventLocation.objects.get_or_create(
            ...
            defaults={
                ...
                'url': 'https://example.com/london-venue'
            }
        )
        ...
        OccurringRule.objects.create(
            event=past_event,
            dt_start=now - timezone.timedelta(days=90),
            dt_end=now - timezone.timedelta(days=87)
        )
        ...
        OccurringRule.objects.create(
            event=sprint_event,
            dt_start=now + timezone.timedelta(days=7),
            dt_end=now + timezone.timedelta(days=9)
        )
        ...
        OccurringRule.objects.create(
            event=pycon_europe,
            dt_start=now + timezone.timedelta(days=120),
            dt_end=now + timezone.timedelta(days=125)
        )
        ...
        OccurringRule.objects.create(
            event=ml_workshop,
            dt_start=now + timezone.timedelta(days=14),
            dt_end=now + timezone.timedelta(days=16)
        )
        ...
        self.stdout.write(self.style.SUCCESS(f'Successfully created test events across {Calendar.objects.count()} calendars'))
        ...
        self.stdout.write(f'Events with recurring rules: {Event.objects.filter(recurring_rules__isnull=False).distinct().count()}')
        self.stdout.write(f'Events with single occurrences: {Event.objects.filter(occurring_rule__isnull=False).count()}')