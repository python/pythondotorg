...
class JobsViewTests(TestCase):
    def setUp(self):
        ...
        self.job_category = JobCategoryFactory(name="Game Production", slug="game-production")

        self.job_type = JobTypeFactory(name="FrontEnd Developer", slug="frontend-developer")
    ...
    def test_job_mine_remove_login(self):
        url = reverse("jobs:job_remove", kwargs={"pk": self.job.pk})
        response = self.client.get(url)
        self.assertRedirects(response, f"/accounts/login/?next=/jobs/{self.job.pk}/remove/")
    ...
    def test_job_edit(self):
        ...
        self.assertRedirects(response, f"/jobs/{job.pk}/preview/")
        ...
        self.assertRedirects(response, f"/accounts/login/?next=/jobs/{job.pk}/edit/")
    ...
    def test_job_create(self):
        ...
        self.assertEqual(mail.outbox[0].subject, f"Job Submitted for Approval: {job.display_name}")
    ...
    def test_job_categories(self):
        job_category2 = JobCategoryFactory(name="Web Development", slug="web-development")
    ...
    def test_job_display_name(self):
        self.assertEqual(self.job.display_name, f"{self.job.job_title}, {self.job.company_name}")
...
class JobsReviewTests(TestCase):
    def setUp(self):
        ...
        User = get_user_model()
        self.creator = User.objects.create_user(self.creator_username, self.creator_email, self.creator_password)

        self.superuser = User.objects.create_superuser(self.super_username, self.super_email, self.super_password)

        self.another = User.objects.create_user(self.another_username, self.another_email, self.another_password)
    ...
    def test_job_comment(self):
        ...
        self.assertIn("There is a new review comment available for your job posting.", mail.outbox[0].body)
