...
class PEPConverterTests(TestCase):
    ...
    def test_invalid_pep_number(self):
        ...
        self.assertRegex(stdout.getvalue(), r"PEP Path '(.*)9999999(.*)' does not exist, skipping")

    def test_add_image_not_found(self):
        with captured_stdout() as stdout:
            add_pep_image(FAKE_PEP_REPO, "0525", "/path/that/does/not/exist")
        self.assertRegex(stdout.getvalue(), r"Image Path '(.*)/path/that/does/not/exist(.*)' does not exist, skipping")
