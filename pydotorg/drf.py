...
class BaseFilterSet(filters.FilterSet):
    ...
    def qs(self):
        ...
        for param in set(self.data) - set(self.filters):
            ...
            errors.append(f"{filter!r} is not an allowed filter on the {field!r} field.")
        if errors:
...
class BaseAPITestCase:
    ...
    def _check_testcase_config(self):
        if self.api_version is None:
            raise ImproperlyConfigured("Please set 'api_version' attribute in your test case.")
        if self.app_label is None:
            raise ImproperlyConfigured("Please set 'app_label' attribute in your test case.")
